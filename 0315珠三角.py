...
# 更新输出文件夹路径
OUTPUT_DIR = BASE + r'\0315GINItestoutput_maps_WithHKMC'
...
# 1. 空间数据加载与掩模构建 (全域保留)
# =============================
print('>>> [1/4] 启动空间数据读取与粤港澳大湾区掩模初始化...')
arrays = {}
...
# 3. 批量生成并渲染三大维度基尼地图
# =============================
print('>>> [2/4] 计算并渲染空间基尼映射图...')

# --- SMOD ---
print('  -> 处理 城乡功能区 (SMOD) ...')
smod_global, smod_nested = generate_gini_maps(arrays['SMOD'])
plot_gini_surface(smod_global, '大湾区全域(十一城) 城乡功能区(SMOD) 总体极化映射图', 'Map_SMOD_Global_Gini.png')
...
# 修正文件命名为GBA11
matrix_smod.to_csv(os.path.join(OUTPUT_DIR, 'GBA11_MeanGDP_SMOD.csv'), encoding='utf-8-sig')
matrix_lcz.to_csv(os.path.join(OUTPUT_DIR, 'GBA11_MeanGDP_LCZ.csv'), encoding='utf-8-sig')
...
plt.title('大湾区十一城各城乡功能区(SMOD)平均经济产出(GDP)异质性', fontsize=15, pad=15)
plt.xlabel('SMOD 类型代码', fontsize=12)
plt.ylabel('城市', fontsize=12)
plt.xticks(rotation=0)
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_DIR, 'Fig_GBA11_MeanGDP_SMOD_Matrix.png'), dpi=300)
plt.close()
...
plt.title('大湾区十一城各局部气候分区(LCZ)平均经济产出(GDP)异质性', fontsize=16, pad=20)
plt.xlabel('局部气候分区 (LCZ 1-17)', fontsize=12)
plt.ylabel('城市', fontsize=12)
plt.xticks(rotation=45, ha='right')
plt.yticks(rotation=0)
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_DIR, 'Fig_GBA11_MeanGDP_LCZ_Matrix.png'), dpi=300)
plt.close()
